#!/usr/bin/env python3
"""
Legacy SkillPoison experiment runner.

The presentation-aligned entry points are now:
  - decision PoC: phase3-experiments/runners/decision_eval.py
  - real agent A/B/C execution: phase3-experiments/runners/orchestrator.py

This file is kept for older synthetic-taxonomy experiments.
"""
...
def main():
    ...
    if stage in {"decision-poc", "closed-loop"}:
        print("이 설정은 발표자료 기준 SkillProbe 스키마입니다.")
        print("decision PoC는 다음 명령을 사용하세요:")
        print(f"  python3 phase3-experiments/runners/decision_eval.py --config {args.config} --dry-run")
        print("real-agent A/B/C 실행은 orchestrator.py와 Stage 02 설정을 사용하세요.")
        return
