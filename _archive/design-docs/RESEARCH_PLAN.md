...
> 2026-05-17 정렬 메모: 이 문서는 초기 `SkillPoison` 연구 계획 기록이다. 현재 프로젝트 구조와 실행 우선순위는 발표자료의 `SkillProbe` 설계에 맞춰 조정했다. canonical RQ와 4-stage pipeline은 `README.md`, `docs/SKILLPROBE_ALIGNMENT.md`, `configs/pilot.yaml`, `configs/experiment.yaml`을 기준으로 한다.
