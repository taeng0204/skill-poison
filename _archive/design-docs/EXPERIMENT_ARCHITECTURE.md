...
> 2026-05-17 정렬 메모: 이 문서는 초기 설계 기록으로 보존한다. 현재 canonical 실험 구조는 `docs/presentation/`의 SkillProbe 발표자료와 `docs/SKILLPROBE_ALIGNMENT.md`를 따른다. 특히 우선순위는 synthetic L1~L5 Skill 매트릭스가 아니라 `docs/selected_skills/` 85개 실제 Skill 기반 decision PoC, A/B/C stage-wise agent 실행, XAI, Self vs Proxy 방어 재평가다.
