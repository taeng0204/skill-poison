...
```bash
python3 experiment/decision_eval/decision_eval.py --config configs/pilot.yaml --dry-run
python3 experiment/decision_eval/decision_eval.py --config configs/pilot.yaml --write-prompts
python3 experiment/decision_eval/decision_eval.py --config configs/pilot.yaml --run
```
...
`experiment/verdict/engine.py` still keeps the older composite fields (`asr`, `drr`, `task_completed`) for compatibility, but now also emits `stage_a_asr`, `stage_b_asr`, and `stage_c_asr`.
...
JSON verdict under `experiment/runner/results/`.

For quick model and prompt checks, use the Docker convention:

```bash
experiment/docker/opencode/run.sh models/qwen3.env smoke.txt
```

To add a model, add `experiment/docker/opencode/models/<name>.env`.
To add a prompt, add `experiment/docker/opencode/prompts/<name>.txt`.
...
```bash
docker compose -f experiment/docker/docker-compose.yml --profile local-model up -d canary ollama
docker compose -f experiment/docker/docker-compose.yml build agent-opencode
...
`_archive/poc-iterations/OPENCODE_EXTENSION_GUIDE.md`

Current local findings:

`_archive/poc-iterations/OPENCODE_TOOLCALL_RESULTS.md`
