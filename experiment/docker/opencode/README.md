...
```bash
experiment/docker/opencode/run.sh models/new-model.env smoke.txt
```
...
```bash
experiment/docker/opencode/run.sh models/qwen3.env my-prompt.txt
```

For full PoC verdicts, add the same prompt file to `_archive/poc-iterations/opencode_prompts.json` with `expected_files`.
